...
func ExampleSubtreeSpecification_roundTripDER() {
	spec := `{base "n=1,n=4,n=1,n=6,n=3,n=1", specificExclusions { chopBefore "n=14", chopAfter "n=555", chopAfter "n=74,n=6" }, minimum 1, maximum 1, specificationFilter or:{item:organization,not:item:2.5.6.9,and:{item:person,item:2.5.6.14}}}`

	sub, err := NewSubtreeSpecification(spec)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	fmt.Println(dec)
	// Output: {base "n=1,n=4,n=1,n=6,n=3,n=1", specificExclusions { chopBefore "n=14", chopAfter "n=555", chopAfter "n=74,n=6" }, minimum 1, maximum 1, specificationFilter or:{item:organization,not:item:2.5.6.9,and:{item:person,item:2.5.6.14}}}
}

func ExampleRefinement_toFilter() {
	spec := `{base "n=1,n=4,n=1,n=6,n=3,n=1", specificExclusions { chopBefore "n=14", chopAfter "n=555", chopAfter "n=74,n=6" }, minimum 1, maximum 1, specificationFilter or:{item:organization,and:{item:person,item:2.5.6.14},not:item:2.5.6.9}}`
	...
	var f Filter
	if f, err = NewFilter(sub.SpecificationFilter); err != nil {
		fmt.Println(err)
		return
	}
	fmt.Println(f)
	// Output: (|(objectClass=organization)(&(objectClass=person)(objectClass=2.5.6.14))(!(objectClass=2.5.6.9)))
}

func BenchmarkSubtreeSpecificationASN1(b *testing.B) {
	var subs []SubtreeSpecification
	for i := 0; i < len(testSubSpecs); i++ {
		spec, _ := NewSubtreeSpecification(testSubSpecs[i])
...
func BenchmarkSubtreeSpecificationParse(b *testing.B) {
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		_, _ = NewSubtreeSpecification(testSubSpecs[i%maxIdx])
	}

}

func TestSubtreeSpecification_codecov(t *testing.T) {
	NewSubtreeSpecification(nil)
	NewSubtreeSpecification(``)
	NewSubtreeSpecification(`X`)
	NewSubtreeSpecification(byte(33))
	NewSubtreeSpecification(`{base "n=123456,n=1,n=4,n=1,n=6,n=3,n=1", minimum -1, maximum 1, specificationFilter or:{item:2.5.6.5,not:item:2.5.6.4,and:{item:person,item:2.5.6.14}}}`)
