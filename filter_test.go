...
func ExampleFilterAnd_Index() {
	f, _ := NewFilter(`(&(|(sn=Lučić)(employeeID=123456789))(objectClass=person))`)
...
func ExampleFilterNot_Index() {
	f, _ := NewFilter(`(!(&(objectClass=employee)(terminated=TRUE)))`)
	...
	for idx, x := range []struct {
		...
		*/
		...
		},
		{
			Input:  struct{}{},
			Output: ``,
			Error:  `Incompatible input type for Search Filter`,
			Choice: `invalid`,
			Length: 0,
		},
	} {
		filter, err := NewFilter(x.Input)
		if err != nil {
...
func TestFilter_codecov(t *testing.T) {
	...
	gEqual.isFilter()
	gEqual.Desc = []byte(`n`)
	gEqual.Value = AssertionValue(`test`)
	gEqual.Len()
	gEqual.Index(9)
	gEqual.IsZero()
	...
	lEqual.isFilter()
	lEqual.Desc = []byte(`n`)
	lEqual.Value = AssertionValue(`test`)
	lEqual.Len()
	lEqual.Index(9)
	lEqual.IsZero()
	...
	_ = exts.String()
	e, _ := NewFilter(`(a:dn:1.2.3:=John)`)
	e.(FilterExtensibleMatch).Len()
	e.(FilterExtensibleMatch).Index(9)
	e.(FilterExtensibleMatch).IsZero()

	_ = isOIDOrDescr([]byte(``))
	_ = descrSyntaxCheck([]byte(`abc123-`))
	_ = descrSyntaxCheck([]byte(`-abc123`))
	_ = descrSyntaxCheck([]byte(`abc--123`))
	_ = descrSyntaxCheck([]byte(`abc#123`))

	_ = oIDSyntaxCheck([]byte(`1.3.-6`))
	_ = oIDSyntaxCheck([]byte(`1.3.6.-4`))
	_ = oIDSyntaxCheck([]byte(`1.4A`))
	_ = oIDSyntaxCheck([]byte(`1.41`))
	_ = oIDSyntaxCheck([]byte(`3.4`))
	_ = isValidArc([]byte(`a`))
	_ = isValidArc([]byte(`06`))
	_ = isValidArc([]byte(`?`))
	_ = isValidArc([]byte(`-4`))

	var descr AttributeDescription = []byte("cn")
	_ = descr.Type()
	...
	_ = eqly.String()
	eqly.Desc = []byte(`n`)
	eqly.Value = AssertionValue(`test`)
	eqly.Index(9)
	eqly.IsZero()
	eqly.Len()
	eqly.isFilter()
	...
	_ = aprx.String()
	aprx.Desc = []byte(`n`)
	aprx.Value = AssertionValue(`test`)
	aprx.Index(9)
	aprx.IsZero()
	aprx.Len()
	aprx.isFilter()
	...
	checkParenEncaps([]byte(`(bdf`), []byte(`fdhjds`))
	checkParenEncaps([]byte(`bdf`), []byte(`fdhjds)`))
	checkParenEncaps([]byte(`bdf`), []byte(`fdhjds`))
	checkParenEncaps([]byte(`Ibdf`), []byte(`fdhjds)`))

	// antipanic checks
	checkFilterOIDs([]byte(`at;bogus-tag`), []byte(`i`))
	checkFilterOIDs([]byte(`at`), []byte(`_lr`))
	checkFilterOIDs([]byte(`at`), []byte(``))
	checkFilterOIDs([]byte(`1.3.5`), []byte(``))
	checkFilterOIDs([]byte(`1.3.5`), []byte(`i`))
	checkFilterOIDs([]byte(``), []byte(`1.3.5`))
	checkFilterOIDs([]byte(`at`), []byte(`1.3.5`))
	checkFilterOIDs([]byte(``), []byte(`lr`))
	checkFilterOIDs([]byte(``), []byte(``))
	checkFilterOIDs([]byte(`%$^&@#`), []byte(`#^@`))

	parseItemFilter([]byte(`4783`))
	parseItemFilter([]byte("(something=value"))
	parseItemFilter([]byte(`47=83`))
	parseItemFilter([]byte(`47=_83`))
	parseItemFilter([]byte(`a:dn:1.2.3:=John`))
	parseExtensibleMatch([]byte(`a:dn:1.2.3.4`), []byte(`xxxx`))
	parseFilterNot([]byte(`4`))
	parseFilterNot([]byte(`uifeds\f43829`))
	marshalFilter([]byte(`uifeds\f43829`))
	marshalFilter([]byte(` `))
	parseComplexFilter([]byte(`_`), `&`)

	dnAttrSplit([]byte(`A:dn:Z`))
	dnAttrSplit([]byte(`A:DN:Z`))
}
...
func BenchmarkFilterParse(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _ = NewFilter(filters[i%maxIdx])
	}
}
